...
async function getFirstPet(username) {
  // get the first ten pets from the API
  const response = await axios.get(`${BASE_API}/get_new_recommendation/000/cat?option=collab&animal_id=58698691`)
...
export const Home = function () {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const petFromDB = await getFirstPet(user.email);
